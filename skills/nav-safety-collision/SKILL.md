...
- **地图质量影响决策**：高覆盖率不等于拓扑正确（开环投影陷阱）；占用比过低也不等于墙线成功。
- **跟踪不等于绕障**：画面伺服不会绕体积；全局图未过门时用滚动里程计局部代价图，不要绑未验收的 `map` 帧。
- **转向也要包络**：只查前进带会在原地转时刮侧向；排除机体半径内自碰后再放 yaw。
...
| 低矮 3D 雷达漏门 | 点云压成 2D scan 或高度带不含门 | 局部障碍用机体坐标 PointCloud2，急停扇区排除地面 |
| 指令速度过死区仍几乎不动 | 执行层固件死区，或膨胀/未知格挡住 | 开环定速测实效速度，与 Nav2 到位分开验收 |
| 进未知区撞墙 | unknown 当 free | 查 coverage 策略与 step_m |
| 倒车仍撞、雷达「有 360」 | 身后身高带零点，闸把空圆当自由 | 数身后带内点数；无点则查地图占用，不要信空局部栅格 |
...
| 多层同时大改 | 无法归因 | 单层 A/B |
| 未做冒烟长跑 | 事故代价高 | 短距离验证 |
| 只查前进带、原地转刮障 | 侧向椅腿不在前后扇区 | 有角速度时查包络外圈；忽略机体半径内自碰 |
| 等人跟全局图过门再跟人 | 建图未达标会无限期挡住跟踪 | 滚动 `odom` 局部代价图 + 机体 footprint；与建图互斥 |
| 把被跟的人当障碍清零速度 | 雷达打到人体就停 | 仅对锁定目标小圆柱豁免，家具不豁免 |
| 把未过墙线的房间 `/map` 当 NavigateToPose | 扇骨栅格不是可绕障拓扑 | 跟随走滚动 odom 局部代价图；全局规划等表 1 墙线过门 |
| 稀疏雷达身后无点就当可倒车 | 近平面雷达打不到身高带时，未知被当成自由 | 无回波不要放行；有身高带回波或地图身后占用才刹倒车 |
| 身后完全看不见就全程清零倒车 | 点云一稀或短中断会把步态卡死 | 圆内只清撞上的轴；点云短中断沿用上一帧，过期再全零 |
| 圆内任意占用就急停所有轴 | 侧向障碍会挡住前进/倒车，人觉得「不是 360」或「卡住」 | 按运动轴分别闸；机体自碰半径内忽略 |
